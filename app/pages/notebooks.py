...
def notebooks() -> rx.Component:
    return rx.el.div(
        ...
        rx.el.main(
            rx.el.div(
                rx.el.h1("Notebooks", class_name="text-4xl font-bold text-gray-900"),
                rx.el.p(
                    "Potencia y portabilidad para tu trabajo y entretenimiento.",
                    class_name="text-lg text-gray-600 mt-2",
                ),
                class_name="text-center py-16 bg-green-50 rounded-xl",
            ),
            rx.el.div(
                rx.el.div(
                    rx.el.input(
                        placeholder="Buscar en notebooks...",
                        on_change=MainState.set_search_query.debounce(300),
                        class_name="w-full md:w-1/3 p-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500 focus:outline-none",
                    ),
                    rx.el.select(
                        ...
                        class_name="p-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500 focus:outline-none",
                    ),
                    rx.el.div(
                        rx.el.input(
                            type="number",
        ...
        on_mount=lambda: MainState.set_filter_category("notebook"),
    )